...
class BaseMetricClassifier(BaseFewShotClassifier):
    ...
    # 在 meta-testing阶段(模型验证阶段)的所有步骤都是通过 调用 after_iter_hook 实现的，验证流程主要依托于MetaTestEvalHook类的 evaluate()函数。 在 evaluate()方法中，调用了 test API 方法 single_gpu_meta_test(), 該方法内部依次调用model 的如下方法：

    # 0：meta test阶段的前置工作，用backbone抽取样本嵌入 或 冻结backbone参数
    ...
    # 1: meta test开始的第一步，调用self.head.before_forward_support()， 该方法将 在self.head 对象中保存的 上一次验证阶段中存留的 suuport样本 前向传播结果 
    ...
    # 2：随后会调用forward_support()方法，该方法实际上在forward()内部被调用。计算出 support 样本的嵌入，并在 该方法中 调用 self.head.forward_support 方法， 该方法将 support的嵌入与 gt_label 保存在了 self.head.support_feats、self.head.support_labels中。 在 计算query阶段， 会访问该成员， 计算prototypes 用以 query分类。
    
    # 3: 在 调用forward_support() 方法后，调用 before_forward_query，在内部调用self.head.before_forward_query计算prototypes, 存放在self.head.mean_support_feats中。  
    ...
    # 4: 随后会调用forward_qeury()方法，该方法实际上在forward()内部被调用。计算出 qeury 样本的嵌入，并在 该方法中 调用 self.head.forward_qeury 方法。 该方法访问 self.head.mean_support_feats 中的prototypes，计算softmax pred。