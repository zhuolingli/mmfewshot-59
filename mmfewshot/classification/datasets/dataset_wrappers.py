...
# 将str 的 wnid 转换为可以通过 collect function的 np array
def wnid2np(wnid: str=None):
    pos = wnid[0]
    pos_asc = ord(pos)
    offset = int(wnid[1:])
    wnid = np.array([pos_asc, offset])
    return wnid
...
class EpisodicDataset:
    ...
    def generate_episodic_idxes(self) -> tuple[list[Mapping], list[list[int]]]:
        ...
        class_ids = [i for i in range(len(self.CLASSES))] # class_idx 相当于 gt_label
        # using same episodes seed can generate same episodes for same dataset
        # it is designed for the reproducibility of meta train or meta test
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        if self._with_cache_feats:
            return {
                ...
                # 自定义添加了 wnid 字段， 按需屏蔽
                # 'wnid':wnid2np(self.dataset.data_infos[idx]['img_info']['filename'][0:9])
            }
        else:
