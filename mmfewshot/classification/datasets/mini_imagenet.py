...
class MiniImageNetDataset(BaseFewShotDataset):
    ...
    # 一般在继承自`BaseFewShotDataset`的子类中，需要重写 get_classes 方法，且该方法的传入参数classes 一般为 None。
    # 在 get_classes 方法内部, 根据 dataset 的潜在任务类型(self.subset)调用 预定义的类变量 自行生成 classes_names
    def get_classes(
            self,
            ...
            with open(ann_file) as f:
                for i, line in enumerate(f):
                    ...
                    info = {
                        ...
                        # 'wnid': class_name,   # 这是我自己自定义的，附加了wnid 为每个数据样本
                    }
