...
def single_gpu_meta_test(model: Union[MMDataParallel, nn.Module],
    ...
    model.before_meta_test(meta_test_cfg)  # 执行 model中的backbone.forward 提取所有样本的embed用以meta testing；或者将model.backbone中的参数冻结。
...
def extract_features_for_fast_test(model: MetaTestParallel,
    ...
    with torch.no_grad():
        for i, data in enumerate(test_set_dataloader):
            img_metas_list.extend(data['img_metas'].data[0])  # data['img_metas].data[0] 涵盖了该 batch 中的 img 的 meta info.
...
def test_single_task(model: MetaTestParallel, support_dataloader: DataLoader,
    ...
    if support_cfg.get('train', False):
        ...
        dataloader_iterator = iter(support_dataloader) # support_dataloader 被设置为了`drop_last`。举个例子，5w5s的task，一共25个样本，采取batch_size 4 的方式读入，len(support_dataloader)=4, 最后一个support 样本被舍弃，我觉得不对。
